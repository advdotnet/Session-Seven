...
namespace SessionSeven.Basement
{
	...
	{
		private readonly Rectangle _openedHotspot = new Rectangle(636, 73, 14, 152);
		private readonly Rectangle _closedHotspot = new Rectangle(650, 70, 54, 160);
		...
		public bool Open
		{
			get => Get<Sprite>().Visible;
			private set => Get<Sprite>().Visible = value;
		}

		private Script OnUse(InteractionContext context)
		{
			IEnumerator script;
			...
			using (Game.CutsceneBlock())
			{
				...
				else
				{
					...
					else
					{
						...
						Open = true;
						Get<HotspotRectangle>().SetRectangle(_openedHotspot);
		...
		public IEnumerator CloseScript(bool reset = true)
		{
			...
			using (Game.CutsceneBlock(true, reset))
			{
				...
				else
				{
					...
					Open = false;
					Get<HotspotRectangle>().SetRectangle(_closedHotspot);
