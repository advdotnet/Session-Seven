...
namespace SessionSeven.Basement
{
    ...
    {
        const int BLANKET_FRAME = 1;
        const int FLASHLIGHT_FRAME = 2;
        ...
        IEnumerator LookScript()
        {
            ...
            {
                if (BLANKET_FRAME == Get<Sprite>().CurrentFrame)
                {
                    yield return Game.Ego.Say(Basement_Res.Looks_like_our_old_picnic_blanket);
                }
                else if (FLASHLIGHT_FRAME == Get<Sprite>().CurrentFrame)
                {
        ...
        IEnumerator PickScript()
        {
            ...
            using (Game.CutsceneBlock())
            {
                ...
                if (BLANKET_FRAME == Get<Sprite>().CurrentFrame)
                {
                    Get<Sprite>().CurrentFrame = FLASHLIGHT_FRAME;
