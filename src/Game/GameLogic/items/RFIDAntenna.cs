...
namespace SessionSeven.InventoryItems
{
	...
	{
		protected override RFIDAntennaFloor AntennaFloor => Tree.Basement.RFIDAntennaFloorCabinet;

		protected override RFIDAntenna OtherAntennaInv => Tree.InventoryItems.RFIDAntennaShelf;
	}

	[Serializable]
	public class RFIDAntennaShelf : RFIDAntenna
	{
		protected override RFIDAntennaFloor AntennaFloor => Tree.Basement.RFIDAntennaFloorShelf;

		protected override RFIDAntenna OtherAntennaInv => Tree.InventoryItems.RFIDAntennaCabinet;
	}
	...
	public abstract class RFIDAntenna : ItemBase
	{
		...
		public bool HitCollider => Placed && AntennaFloor.Get<TracerLine>().HitCollider;
		...
		public IEnumerator UseScript(bool reset, bool doCheck)
		{
			using (Game.CutsceneBlock(reset))
			{
				...
				if (AntennaFloor.WasPlacedTooClose && !_selectionAborted)
				{
				...
				if (!_selectionAborted || wasPlaced)
				{
		...
		private IEnumerator PlaceScript(RFIDAntennaFloor antennaFloor, int mode)
		{
			...
			yield return selection.StartSelectionScript(Game.Ego.Get<Scripts>(), antennaFloor, true, mode);
