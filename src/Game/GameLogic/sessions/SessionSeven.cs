...
namespace SessionSeven.Cutscenes
{
	...
	public partial class Director : Entity
	{
		...
		/// </summary>
		/// <returns></returns>
		private IEnumerator SessionSevenScript()
		{
			Game.StopSong();
			...
			Game.StopSkipping();
			StackGame.Engine.Pause();
			StackGame.Engine.Game.UnloadWorld();
			Game.MainMenu.Show();
			StackGame.Engine.Renderer.GUIManager.ShowSoftwareCursor = true;
		}

		private IEnumerator CynthiaDialogScript()
		{
			if (!ForgaveCynthia)
			{
		...
		public static IEnumerable<string> GetCreditTexts(bool ending)
		{
			...
			var newLineNewLine = newLine + newLine;

			yield return GlblRes.Writing + newLineNewLine + GlblRes.Jennifer_Jussel;
			yield return GlblRes.Music + newLineNewLine + GlblRes.Damian_Potenzoni;
			yield return GlblRes.Coding + newLineNewLine + GlblRes.Jonas_Jelli;
			yield return GlblRes.Graphics + newLineNewLine + GlblRes.Jeremy_Carver__Jonas_Jelli;
			yield return GlblRes.Polish_Translation + newLineNewLine + GlblRes.MaciejKubiszewski_;
			if (ending)
			{
				yield return GlblRes.BetaTesting + newLineNewLine + GlblRes.LauraHubbesSimonTschirnerStefanDienstTanjaDienstThorstenKraemer;
				yield return GlblRes.SpecialThanks + newLineNewLine + GlblRes.JenniferThanksFamilyFriendsTheirUnendingSupportSessionSevenTeamMakingThisDreamRealityThanksPlayerTooNoneThisPossibleWithoutYou;
				yield return GlblRes.SpecialThanks + newLineNewLine + GlblRes.DamianWouldLikeThankFamilyFriendsContinuousSupportThanksEveryoneInvolvedSessionSevenMakingPartExperienceGuysRockThanksWifeWithoutHerCouldnTMakeThisPossibleLove;
				yield return GlblRes.SpecialThanks + newLineNewLine + GlblRes.JonasWifeFamilyFriendsEverybodyContributingMakingThisGame;
				yield return GlblRes.SpecialThanks + newLineNewLine + GlblRes.JeremyAmandaChristianCarverMariaSmithPierreBezuidenhoutDawidFrederikStrauss;
				yield return GlblRes.SpecialThanks + newLineNewLine + GlblRes.thanks;
			}
